...
TEST(RecyclebinGetHandlerTest, NoUser){
    ...
    struct http_message* hmsg = new_http_message("GET", "/files/gabriel/recycle_bin/","", &header_names, &header_values);
...
TEST(RecyclebinGetHandlerTest, testOk){
    Database* db = new DatabaseMockRAM;
    TokenAuthenticator* tk = new TokenAuthenticator;
    string token = tk->createToken("gabriel");
    RecyclebinGetHandler handler(db, tk);
